...
def gini_engine(matrix: list) -> dict:
    """
    Discover optimal trading rules through exhaustive feature combination analysis.

    Analyzes the in-memory feature matrix to find multi-feature trading rules
    for each target variable (3-day, 90-day, 180-day, 365-day outlook):
    ...
    Returns:
        dict: Keys are target names ("Target", "Target_90", "Target_180", "Target_365").
              Each value is a dict mapping rule identifiers to serialized Value dicts.
    """
    ...
    for target_col, config in TARGET_CONFIG.items():
        ...
        all_results[target_col] = final_best_combinations
        print(f"[{target_col}] {len(final_best_combinations)} rules discovered.")
...
if __name__ == "__main__":
    ...
    for target_col, rules in gini_results.items():
